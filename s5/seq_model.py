...
class AutoregressiveModel(nn.Module):
    """ S5 autoregressive model. This consists of the stacked encoder
    (which consists of a linear encoder and stack of S5 layers), and a linear decoder.
            ssm         (nn.Module): the SSM to be used (i.e. S5 ssm)
            d_output     (int32):    the output dimension, i.e. the number of classes
            d_model     (int32):    this is the feature size of the layer inputs and outputs
                        we usually refer to this size as H
...
class AutoencoderModel(nn.Module):
    """ S5 autoencoder model. This consists of the stacked encoder
    (which consists of a linear encoder and stack of S5 layers), and a linear decoder.
            ssm         (nn.Module): the SSM to be used (i.e. S5 ssm)
            d_output     (int32):    the output dimension, i.e. the number of classes
            d_model     (int32):    this is the feature size of the layer inputs and outputs
                        we usually refer to this size as H
            d_latent    (int32):    the latent dimension
            n_layers    (int32):    the number of S5 layers to stack
            padded:     (bool):     if true: padding was used
            activation  (string):   Type of activation function to use
            dropout     (float32):  dropout rate
    ...
    def setup(self):
        ...
        Initializes the S5 stacked encoder and a linear decoder.
        """
        self.encoder_ssm = StackedEncoderModel(
                            ssm=self.ssm,
                            d_model=self.d_model,
                            n_layers=self.n_layers,
                            activation=self.activation,
        ...
        self.encoder_output = nn.Dense(self.d_latent)
        
        self.decoder_ssm = StackedEncoderModel(
                            ssm=self.ssm,
                            d_model=self.d_model,
                            n_layers=self.n_layers,
                            activation=self.activation,
    ...
    def __call__(self, x, integration_timesteps, return_latents=False):
        ...
        x = self.encoder_ssm(x, integration_timesteps)
        latents = self.encoder_output(x)
        x = self.decoder_ssm(latents, integration_timesteps)
